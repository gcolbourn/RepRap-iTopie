...
module rear_triangle_fingers_bolts() {
    translate([-feet_peg_size[1] , 20 + bolt_mount_spacing, 0])
        bolt_pegs_triangles();
    translate([-feet_peg_size[1], (_triangle_height - 20 + feet_peg_size[0]) / 2 + bolt_mount_spacing / 2, 0])
        bolt_pegs_triangles();
...
module LCD_cable_through_hole_triangle(){
    translate([8, 0, 0])
    square([20,10]);
}
...
module rear_triangles() {
    translate([3, sheet_thickness/2 + total_feet_height - triangle_margin[0] - feet_pocket_size[1] - 35, 0]) {
        translate([-3*sheet_thickness/3 + vertical_plate_borders[3] + feet_pocket_size[1] + triangle_margin[1], 0, 0]){
            difference(){
            rear_triangle();
